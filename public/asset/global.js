...
if(!data||!data.transitioning){if($parent)$parent.find('[data-toggle="collapse"][data-parent="'+parent+'"]').not($this).addClass('collapsed').attr('aria-expanded',!1)
...
return($href&&$href.length&&$href.is(':visible')&&$el.is(':visible')&&[[$href[offsetMethod]().top+offsetBase,href]])||null}).sort(function(a,b){return a[0]-b[0]}).each(function(){self.offsets.push(this[0])
...
Plugin.call($spy,data)})})}(jQuery);function falert(message,title,callback){callback=(typeof(callback)=="undefined")?function(){dclose()}:callback;title=(typeof(title)=="undefined")?"消息框":title;var callFn=function(){callback()};easyDialog.open({container:{header:title,content:message,yesFn:function(){dclose()}},callback:callFn,overlay:!0})}
function showmsg(msg){easyDialog.open({container:{content:msg},autoClose:1000,overlay:!1})}
function fconfirm(message,callback){callback=(typeof(callback)=="undefined")?function(){}:callback;var btnFn=function(){callback();dclose();return !1};easyDialog.open({container:{header:'消息提醒',content:message,yesFn:btnFn,noFn:!0}})}
function dclose(){easyDialog.close()}
...
return cookieValue}};jQuery.removeHtml=function(s){return(s)?jQuery("<p>").append(s).text():""}
function ajax_post(the_url,the_param,succ_callback,ftype,async){ftype=(typeof(ftype)=="undefined")?"jsonp":ftype;async=(typeof(async)=="undefined")?"true":async;$.ajax({type:'POST',cache:!1,url:the_url,data:the_param,success:succ_callback,beforeSend:loading,complete:complete,async:async,dataType:ftype,jsonp:"jsonpcallback",error:function(html){}})}
function ajax_get(the_url,succ_callback,ftype,async){ftype=(typeof(ftype)=="undefined")?"jsonp":ftype;async=(typeof(async)=="undefined")?"true":async;$.ajax({type:'GET',cache:!1,url:the_url,success:succ_callback,beforeSend:loading,complete:complete,dataType:ftype,async:async,jsonp:"jsonpcallback",error:function(html){}})}
function loading(){$("#loading").show()}
function complete(){$("#loading").hide()}
function checkLength(which,maxChars,idName){if(which.value.length>maxChars)
which.value=which.value.substring(0,maxChars);var curr=maxChars-which.value.length;document.getElementById(idName).innerHTML=curr.toString()}
function showModel(url,title){ajax_get(url,function(a){$("#mymodelContent").html(a);$("#myModalLabel").html(title);$('#myModal').modal({keyboard:!0})},'text')}